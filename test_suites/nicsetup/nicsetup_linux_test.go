...
const (
	// Retry the connection a few times to ensure the destination is reachable.
	numRetries = 6
	...
	// pingVMPort is the port to listen on for the ping VM.
	pingVMPort = 80
)
...
func createConnection(t *testing.T, nic managers.EthernetInterface, ipv4Addr, ipv6Addr, port string) {
	...
	defer cancel()
	_, ipv4Err := dialer.DialContext(timeoutCtx, "tcp4", net.JoinHostPort(ipv4Addr, port))
	...
	timeoutCtx, cancel = context.WithTimeout(utils.Context(t), connectionTimeout)
	defer cancel()
	_, ipv6Err := dialer.DialContext(timeoutCtx, "tcp6", net.JoinHostPort(ipv6Addr, port))
				...
				if err != nil {
					...
					go func(conn *net.TCPConn) {
						...
						if numIPv4Connections == expectedConnections {
							t.Logf("%s: Received expected number of IPv4 connections, exiting.", getCurrentTime())
							ipv4Cancel()
							return
						}
				...
				if err != nil {
					...
					go func(conn *net.TCPConn) {
						...
						if numIPv6Connections == expectedConnections {
							t.Logf("%s: Received expected number of IPv6 connections, exiting.", getCurrentTime())
							ipv6Cancel()
							return
						}
	...
	for {
		...
		default:
			...
			if numIPv4Connections >= expectedConnections && numIPv6Connections >= expectedConnections {
				ipv4ConnectionsMu.Unlock()
				ipv6ConnectionsMu.Unlock()
				t.Logf("Received expected number of connections, exiting.")
				return
			}
			ipv4ConnectionsMu.Unlock()
			ipv6ConnectionsMu.Unlock()
