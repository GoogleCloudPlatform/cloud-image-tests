...
func TestGPUNumaMapping(t *testing.T) {
	for i := 0; i < 8; i++ {
		...
		if i < 4 {
			if numaMapping != "0" {
				t.Fatalf("TestGPUNumaMapping: GPU %d has numa node %v, want 0", i, numaMapping)
			}
		} else {
			if numaMapping != "1" {
				t.Fatalf("TestGPUNumaMapping: GPU %d has numa node %v, want 1", i, numaMapping)
			}
		}
	}
}

func TestNICNumaMapping(t *testing.T) {
	ctx := utils.Context(t)
	for i := 0; i < 10; i++ {
		...
		if i == 0 || (i > 1 && i < 6) {
			if numaMapping != "0" {
				t.Fatalf("TestNICNumaMapping: %s (index %d) has numa node %v, want 0", iface.Name, i, numaMapping)
			}
		} else {
			if numaMapping != "1" {
				t.Fatalf("TestNICNumaMapping: %s (index %d) numa node %v, want 1", iface.Name, i, numaMapping)
			}
		}
	}
}
