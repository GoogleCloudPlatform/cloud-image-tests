...
func TestSetup(t *imagetest.TestWorkflow) error {
	// Skipping license check for Windows Client Images (We don't publish them) & openSUSE Leap Images
...
func requiredLicenseList(t *imagetest.TestWorkflow) ([]string, error) {
	...
	case utils.IsRHELEUS(image.Name) || utils.IsRHELLVM(image.Name) || (utils.IsRHEL(image.Name) && utils.IsBeta(image.Name)):
		...
		transform = func() {
			...
			if utils.IsRHELEUS(image.Name) {
				requiredLicenses = append(requiredLicenses, fmt.Sprintf(licenseURLTmpl, project, fmt.Sprintf("rhel-%s-server-eus", rhelMajorVersion)))
			}
