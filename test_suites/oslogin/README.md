...
## Overview
Having a separate test organization set up for this test suite is highly
recommended. This is because having a separate test org:
...
## Test Setup
This test uses multiple secrets to store the test users that it uses in order to
run the test. These secrets are as follows:
...
For the 2FA users, there are 3 for each of normal and admin 2FA users.
...
As a consequence, users of this test suite must be able to set up at least 4
users. The above examples are the minimum required users, but especially in the
case of 2FA, the more you have, the better due to the much lower 2FA quotas.
...
Note that the admin users should have the `roles/compute.osAdminLogin`
permission.

## User Setup
In order to set up each of the users for the test, their OSLogin profiles must
be set up with a permanent public SSH key. This can be accomplished by doing
the following:

1. [Open Cloud Shell](https://docs.cloud.google.com/shell/docs/using-cloud-shell)
...
4. Add the SSH public key to the user's OSLogin Profile.
   ...
   ```
5. Copy and paste the contents of the private key file into the appropriate
   secret, or use the `gcloud` CLI.
   
   For example, you can add the private SSH Key for `normal-user` by calling the
   following:
   ```
...
As the test requires the 2FA secrets, when setting up the 2FA users, it is
important that you acquire the secret before completing the 2FA setup. The steps
are as follows (in addition to the steps above):
...
4. Click the linked text saying that you can't scan the barcode, which reveals
   your 2FA secret. Save this secret into the appropriate secret in GCP's Secret
   Manager.
