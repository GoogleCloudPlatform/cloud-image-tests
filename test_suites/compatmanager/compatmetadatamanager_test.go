...
func verifyMetadataScriptsProcesses(t *testing.T) {
	t.Helper()
	if utils.IsWindows() {
		processExistsWindows(t, true, "GCECompatMetadataScripts")
		processExistsWindows(t, true, "GCEMetadataScriptRunner")
		processExistsWindows(t, false, "GCEMetadataScripts")
	} else {
		processExistsLinux(t, true, "/usr/bin/gce_metadata_script_runner")
		processExistsLinux(t, true, "/usr/bin/gce_compat_metadata_script_runner")
		processExistsLinux(t, false, "/usr/bin/google_metadata_script_runner")
	}
}

func verifyFileOutput(t *testing.T, event string) {
	t.Helper()
	var path string
	var wantProcesses []string
	var dontWantProcess string
	...
	if utils.IsWindows() {
		...
		wantProcesses = []string{"GCEMetadataScriptRunner", "GCECompatMetadataScripts"}
		dontWantProcess = "GCEMetadataScripts"
		processes = strings.TrimSpace(string(data))
	} else {
		path = fmt.Sprintf("/home/%s.txt", event)
		wantProcesses = []string{"/usr/bin/gce_metadata_script_runner", "/usr/bin/gce_compat_metadata_script_runner"}
		dontWantProcess = "/usr/bin/google_metadata_script_runner"
...
func TestMetadataScriptCompatStartup(t *testing.T) {
	...
	verifyMetadataScriptsProcesses(t)
	verifyFileOutput(t, "startup")
}
